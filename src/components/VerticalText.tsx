...
export class SingleTextInformation {
  ...
  constructor(text: string, font: string, color: string, width: number, height: number, relativeUL: Position, isRotated: boolean)
  constructor(text: string, font: string, color: string, width?: number, height?: number, relativeUL?: Position, isRotated?: boolean) {
    this.text = text;
    this.font = font;
    this.color = color;

    if (width !== undefined && height !== undefined && relativeUL !== undefined && isRotated !== undefined) {
      this.width = width;
      this.height = height;
  ...
  public draw(ctx: CanvasRenderingContext2D, verticalTextRelativeUL: Position, mainUL: Position) {
  ...
  public clone() {
    return new SingleTextInformation(this.text, this.font, this.color, this.width, this.height, this.relativeUL, this.isRotated)
  }
  ...
  private halfNumberJustify() {
    ...
    if (this.text.length <= 3) {
      ...
      this.height = baseMetric.actualBoundingBoxAscent + baseMetric.actualBoundingBoxDescent;
    } else {
      this.width = baseMetric.actualBoundingBoxAscent + baseMetric.actualBoundingBoxDescent;
  ...
  private rotatedTextJustfy() {
    SingleTextInformation.ctx.font = this.font;
    const metric = SingleTextInformation.ctx.measureText(this.text);
    this.width = metric.actualBoundingBoxAscent + metric.actualBoundingBoxDescent;
    this.height = metric.width;
    this.isRotated = true;
    this.relativeUL = {
      x: 0, y: 0
  ...
  private smallCharJustfy() {
    SingleTextInformation.ctx.font = this.font;
    const metric = SingleTextInformation.ctx.measureText(SingleTextInformation.baseSmallChar);
    const smallWidth = metric.actualBoundingBoxLeft + metric.actualBoundingBoxRight;
    const smallHeight = metric.actualBoundingBoxAscent + metric.actualBoundingBoxDescent;
    
    const baseMetric = SingleTextInformation.ctx.measureText(SingleTextInformation.baseChar);
    this.width = baseMetric.width
    const baseActualWidth = baseMetric.actualBoundingBoxLeft + baseMetric.actualBoundingBoxRight;
    this.height = baseMetric.actualBoundingBoxAscent + baseMetric.actualBoundingBoxDescent;
    this.relativeUL = {
      x: (baseActualWidth - smallWidth) / 2, y: -(this.height - smallHeight)
    }
  }
  ...
  private punctuationJustify() {
    ...
    this.height = baseMetric.actualBoundingBoxAscent + baseMetric.actualBoundingBoxDescent;
    this.relativeUL = {
      x: baseActualWidth * 3 / 4, y: -this.height * 3 / 4
    }
  ...
  private rotatedCenterCharJustify() {
    SingleTextInformation.ctx.font = this.font;
    this.normalCharJustify();
    const metric = SingleTextInformation.ctx.measureText(SingleTextInformation.baseChar);
    this.width = metric.width;
    this.height = metric.actualBoundingBoxAscent + metric.actualBoundingBoxDescent;
    const baseRotateMetric = SingleTextInformation.ctx.measureText(SingleTextInformation.rotatedCenterCharList.charAt(0));
    const baseHeight = baseRotateMetric.actualBoundingBoxAscent + baseRotateMetric.actualBoundingBoxDescent;
