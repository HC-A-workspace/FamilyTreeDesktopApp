...
function drawLines(ctx: CanvasRenderingContext2D, familyTree: FamilyTree) {
  ...
  for (const [id, ] of marriages) {
    ...
    for (const child of familyTree.findChildrenOfMarriage(id)) {
      const hasAdoptedParentMarriage = child.getAdoptedParentMarriageId() !== undefined
      const childProps: ChildProperties = {
        isAdopted: false,
        topY: child.getTopY() - offset,
        centerX: hasAdoptedParentMarriage ? child.getCenterX() - margin / 3 : child.getCenterX(),
    ...
    for (const child of familyTree.findAdoptedChildrenOfMarriage(id)) {
      ...
      const childProps: ChildProperties = {
        ...
        centerX: hasParentMarriage ? child.getCenterX() + margin / 3 : child.getCenterX(),
        leftX: child.getLeftX() - offset,
        rightX: child.getRightX() + offset
      }
      unsortedChildrenProps.push(childProps);
    ...
    const childrenProps = unsortedChildrenProps.sort((c1, c2) => c1.centerX - c2.centerX);
    ...
    if (numParents === 0) {
      ...
      if (numChildren === 1) {
        ...
        if (parentLeftX + padding < childCenterX && childCenterX < parentRightX - padding && childTopY > parentBottomY) {
          //親一人、子一人、真下
          const v: Vertical = {
            isDouble: isDouble,
            x: childCenterX,
          ...
          if (childCenterX < parentCenterX) {
            ...
            const middleX = Math.min((childCenterX + parentCenterX) / 2, childRightX + margin)
            ...
            const middleX = Math.max((childCenterX + parentCenterX) / 2, childLeftX - margin)
        ...
        if (parentBottomY + margin < allChildrenTopY - margin) {
          if (leftChildCenterX + padding < parentRightX - padding && parentLeftX + padding < rightChildCenterX - padding) {
            //親一人、子複数、垂直距離十分、真下
            const middleX = (Math.max(leftChildCenterX, parentLeftX) + Math.min(rightChildCenterX, parentRightX)) / 2;
            const v: Vertical = {
              isDouble: false,
              x: middleX,
              y1: parentBottomY,
          ...
          if (middleChildX < parentCenterX) {
            ...
            const childX = Math.max(leftChildCenterX + padding, Math.min(parentLeftX - 2 * margin, rightChildCenterX - margin));
            const middleX = Math.min(rightChildRightX + margin, parentLeftX - margin);
            const v1: Vertical = {
              isDouble: false,
              x: parentCenterX,
              y1: parentBottomY,
            ...
            const childX = Math.min(rightChildCenterX - padding, Math.max(parentRightX + 2 * margin, leftChildCenterX + margin));
            const middleX = Math.max(leftChildLeftX - margin, parentRightX + margin);
            const v1: Vertical = {
              isDouble: false,
              x: parentCenterX,
              y1: parentBottomY,
        ...
        for (const childProps of childrenProps) {
          const v: Vertical = {
            ...
            x: childProps.centerX,
            y1: allChildrenTopY - margin,
            y2: childProps.isAdopted ? childProps.topY + margin / 2 : childProps.topY
          }
          verticals.push(v);
        }
      ...
      const [leftParent, rightParent] = parents.sort((p1, p2) => p1.getCenterX() - p2.getCenterX());
      ...
      if (leftParentRightX + margin / 2 < rightParentLeftX - margin / 2) {
        ...
        if (rightParentBottomY + margin / 2 < leftParentTopY - margin / 2) {
          ...
          const middleY = Math.max((leftParentTopY + rightParentBottomY) / 2, leftParentTopY - margin);
          const h1: Horizontal = {
            isDouble: true,
            x1: rightParentLeftX,
            x2: rightParentLeftX - margin,
        ...
        } else if (leftParentBottomY + margin / 2 < rightParentTopY - margin / 2) {
          //二人の親の水平距離が近すぎる、左親が十分に上
          const middleY = Math.max((leftParentTopY + rightParentBottomY) / 2, rightParentTopY - margin);
          const h1: Horizontal = {
            isDouble: true,
            x1: leftParentRightX,
            x2: leftParentRightX + margin,
      ...
      if (numChildren === 1) {
        ...
        if (parentHorizontal.x1 + padding < childCenterX && childCenterX < parentHorizontal.x2 - padding) {
          if (parentHorizontal.y + margin / 2 < childTopY - margin / 2) {
            ...
            if (childCenterX < parentMiddleX) {
              ...
              const childRightX = childrenProps[0].rightX;
              const parentX = Math.min(childRightX + 2 * margin, parentHorizontal.x2 - padding);
              ...
              const childLeftX = childrenProps[0].leftX;
              const parentX = Math.max(childLeftX - 2 * margin, parentHorizontal.x1 + padding);
          ...
          if (childRightX + margin > leftParent.getLeftX() - offset && leftParentBottomY > parentHorizontal.y) {
            if (parentHorizontal.y + margin < childTopY - margin) {
              //親二人、子一人、子が十分に下、子はやや左
              const middleY = (leftParentBottomY + margin / 2 < childTopY - margin / 2) ? Math.max(childTopY - margin, (childTopY + leftParentBottomY) / 2) : childTopY - margin
              const parentX = Math.min(Math.max(parentHorizontal.x1 + margin, childRightX + margin), parentHorizontal.x2 - padding);
              ...
              const parentX = Math.min(Math.max(parentHorizontal.x1 + margin, middleX + margin), parentHorizontal.x2 - padding);
              const v1: Vertical = {
                isDouble: isDouble,
                x: parentX,
                y1: parentHorizontal.y,
            ...
            const parentX = Math.min(Math.max(parentHorizontal.x1 + margin, childRightX + 2 * margin), parentHorizontal.x2 - padding);
            
            if (leftBottomY + margin / 2 < childTopY - margin / 2) {
              //親二人、子一人、子が十分に下、子は左
              const middleY = Math.max(childTopY - margin, (childTopY + leftBottomY) / 2);
          ...
          if (childLeftX - margin < rightParent.getRightX() + offset && rightParentBottomY > parentHorizontal.y) {
            if (parentHorizontal.y + margin < childTopY - margin) {
              //親二人、子一人、子が十分に下、子はやや右
              const middleY = (rightParentBottomY + margin / 2 < childTopY - margin / 2) ? Math.max(childTopY - margin, (childTopY + rightParentBottomY) / 2) : childTopY - margin
              const parentX = Math.max(Math.min(parentHorizontal.x2 - margin, childLeftX - margin), parentHorizontal.x1 + padding);
              ...
              const parentX = Math.max(Math.min(parentHorizontal.x2 - margin, middleX - margin), parentHorizontal.x1 + padding);
              const v1: Vertical = {
                isDouble: isDouble,
                x: parentX,
                y1: parentHorizontal.y,
            ...
            const rightBottomY = Math.max(rightParentBottomY, parentHorizontal.y);
            const parentX = Math.max(Math.min(parentHorizontal.x2 - margin, childLeftX - 2 * margin), parentHorizontal.x1 + padding);
  
            if (rightBottomY + margin / 2 < childTopY - margin / 2) {
              //親二人、子一人、子が十分に下、子は右
              const middleY = Math.max(childTopY - margin, (childTopY + rightBottomY) / 2);
        ...
        if (leftX + offset < rightX - offset) {
          if (parentHorizontal.y + margin < allChildrenTopY - margin) {
            ...
            if (childMiddleX < parentMiddleX) {
              ...
              const middleX = (childrenProps[numChildren - 1].topY - padding > parentHorizontal.y + margin) ? rightChildCenterX + margin : rightChildRightX + margin;
              const parentX = Math.min(middleX + margin, parentHorizontal.x2 - padding);
              ...
              const middleX = (childrenProps[0].topY - padding > parentHorizontal.y + margin) ? leftChildCenterX - margin : leftChildLeftX - margin;
              const parentX = Math.max(middleX - margin, parentHorizontal.x1 + padding);
          ...
          const childX = Math.max(rightChildCenterX - margin, childMiddleX);
          const rightX = (childrenProps[numChildren - 1].topY - padding > leftParentBottomY + margin) ? rightChildCenterX : rightChildRightX;
          
          if (rightX + margin > leftParent.getLeftX() - offset && leftParentBottomY > parentHorizontal.y) {
            if (parentHorizontal.y + margin < allChildrenTopY - 2 * margin) {
              //親二人、子複数、子が十分に下、子はやや左
              const middleY = 
                (leftParentBottomY + margin / 2 < allChildrenTopY - margin - margin / 2)
                  ? Math.max(allChildrenTopY - 2 * margin, (allChildrenTopY - margin + leftParentBottomY) / 2)
                  : allChildrenTopY - 2 * margin
              const parentX = Math.min(Math.max(parentHorizontal.x1 + margin, rightChildCenterX + margin), parentHorizontal.x2 - padding);
              ...
              const childRightX = (childrenProps[numChildren - 1].topY - padding > parentHorizontal.y + margin) ? rightChildCenterX : rightChildRightX;
              const middleX = Math.max(leftParentRightX, childRightX) + margin
              const parentX = Math.min(Math.max(parentHorizontal.x1 + margin, middleX + margin), parentHorizontal.x2 - padding);
              const v1: Vertical = {
                isDouble: false,
            ...
            const leftBottomY = Math.max(leftParentBottomY, parentHorizontal.y);
            const middleX = (childrenProps[numChildren - 1].topY - padding > leftBottomY + margin) ? rightChildCenterX + margin : rightChildRightX + margin;
            const parentX = Math.min(Math.max(parentHorizontal.x1 + margin, middleX + margin), parentHorizontal.x2 - padding);
  
            if (leftBottomY + margin / 2 < allChildrenTopY - margin - margin / 2) {
              //親二人、子複数、子が十分に下、子は左
              const middleY = Math.max(allChildrenTopY - 2 * margin, (allChildrenTopY - margin + leftBottomY) / 2);
          ...
          const childX = Math.min(leftChildCenterX + margin, childMiddleX);
          const leftX = (childrenProps[0].topY - padding > rightParentBottomY + margin) ? leftChildCenterX : leftChildLeftX;
          
          if (leftX - margin < rightParent.getRightX() + offset && rightParentBottomY > parentHorizontal.y) {
            if (parentHorizontal.y + margin < allChildrenTopY - 2 * margin) {
              //親二人、子複数、子が十分に下、子はやや右
              const middleY = 
                (rightParentBottomY + margin / 2 < allChildrenTopY - margin - margin / 2)
                  ? Math.max(allChildrenTopY - 2 * margin, (allChildrenTopY - margin + rightParentBottomY) / 2)
                  : allChildrenTopY - 2 * margin
              const parentX = Math.max(Math.min(parentHorizontal.x2 - margin, leftChildCenterX - margin), parentHorizontal.x1 + padding);
              ...
              const childLeftX = (childrenProps[0].topY - padding > parentHorizontal.y + margin) ? leftChildCenterX : leftChildLeftX;
              const middleX = Math.min(rightParentLeftX, childLeftX) - margin
              const parentX = Math.max(Math.min(parentHorizontal.x2 - margin, middleX - margin), parentHorizontal.x1 + padding);
              const v1: Vertical = {
                isDouble: false,
            ...
            const rightBottomY = Math.max(rightParentBottomY, parentHorizontal.y);
            const middleX = (childrenProps[0].topY - padding > rightBottomY + margin) ? leftChildCenterX - margin : leftChildLeftX - margin;
            const parentX = Math.max(Math.min(parentHorizontal.x2 - margin, middleX - margin), parentHorizontal.x1 + padding);
  
            if (rightBottomY + margin / 2 < allChildrenTopY - margin - margin / 2) {
              //親二人、子複数、子が十分に下、子は右
              const middleY = Math.max(allChildrenTopY - 2 * margin, (allChildrenTopY - margin + rightBottomY) / 2);
